...
$varIdDis_consulta_Viewdistribuidor = "0";
if (isset($_GET['coddis'])) {
  $varIdDis_consulta_Viewdistribuidor = $_GET['coddis'];
}
mysql_select_db($database_conexionmiura, $conexionmiura);
$query_consulta_Viewdistribuidor = sprintf("SELECT * FROM tbldistribuidor WHERE tbldistribuidor.idDistribuidor = %s", GetSQLValueString($varIdDis_consulta_Viewdistribuidor, "int"));
...
<?php $namepais = $row_consulta_Viewdistribuidor['strName']; ?>
